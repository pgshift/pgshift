...
<CardGrid>
  ...
  <Card title="Cron" icon="sun">
    Recurring job scheduling via `pg_cron`. Jobs fire as queue entries -- no extra scheduler process.
  </Card>
...
<Tabs>
  ...
  <TabItem label="Cron">
    ...
    // Every Monday at 8am — send weekly digest
    ...
    // Every 5 minutes
    await cron.cron('heartbeat').schedule('*/5 * * * *', {
